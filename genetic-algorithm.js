...
function initMap() {
  ...
  function calcTotalDistance(waypoints, individual) {
    ...
    for (let i = 0; i < individual.length - 1; ++i) {
      totalDistance += getHaversineDistance(
        waypoints[individual[i]],
        waypoints[individual[i + 1]]
      );
    }
    // Add distance back to origin if returnToOrigin is set to true
    return returnToOrigin === "true"
      ? totalDistance +
          getHaversineDistance(
            waypoints[0],
            waypoints[individual[individual.length - 1]]
          )
      : totalDistance;
  }
  ...
  function genNewPopulation(
    ...
    if (random <= mutChance) {
      ...
      [newIndividual[index1], newIndividual[index2]] = [
        newIndividual[index2],
        newIndividual[index1],
      ];
    }
